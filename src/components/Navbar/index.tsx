              ...
              <div className="border-0 rounded-3xl shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                {signedInPage? <>
                  ...
                  <div>
                      ...
                      <form >
                      {errorMssg && (
                          <p data-aos="flip-down" className="error text-error text-xs font-bold"> {errorMssg} ! </p>
                        )}
                         <div className="bg-white h-full rounded-2xl mt-4 shadow-custom">                          
                          <input className='placeholder-slate-600  p-4 rounded-2xl w-full h-full appearance-none' type="text" name="username" id="username" placeholder="Username" value={newUserName} onChange={(e)=>{setNewUserName(e.target.value)}}/>
                        </div>

                        <div className="bg-white h-full rounded-2xl mt-4 shadow-custom">                          
                          <input className='placeholder-slate-600  p-4 rounded-2xl w-full h-full appearance-none' type="text" name="username" id="username" placeholder="E-mail" value={newEmail} onChange={(e)=>{setNewEmail(e.target.value)}}/>
                        </div>
                        <div className="bg-white h-full rounded-2xl mt-4 shadow-custom" >                          
                          <input 
                          className='placeholder-slate-600  p-4 rounded-2xl w-full h-full appearance-none' 
                          type={`${isPasswordVisible? 'text':'password'}`} name="password" id="password" placeholder="Password" value={newPassword} onChange={(e)=>{setNewPassword(e.target.value)}}/>                               
                        </div>
                        
                        <div className='w-full  items-center justify-center flex'>
                          <Button text='Register' onClick={()=>registerFunction()}></Button>
                          </div>
                      </form>
                ...
                <div className="relative p-6 w-full flex-col bg-[#EBEAF5] rounded-b-3xl">
                  <div>
                      ...
                      {errorMssg && (
                        ...
                        <div className="bg-white h-full rounded-2xl mt-4 shadow-custom">                          
                          <input className='placeholder-slate-600  p-4 rounded-2xl w-full h-full appearance-none' type="text" name="username" id="username" placeholder="E-mail" value={email} onChange={(e)=>{setEmail(e.target.value)}}/>
                        </div>
                        <div className="bg-white h-full rounded-2xl mt-4 shadow-custom" >                          
                          <input 
                          className='placeholder-slate-600  p-4 rounded-2xl w-full h-full appearance-none' 
                          type={`${isPasswordVisible? 'text':'password'}`} name="password" id="password" placeholder="Password" value={password} onChange={(e)=>{setPassword(e.target.value)}}/>      
                      ...
                      <div className="social-message">
                        ...
                        <p className="message font-bold">Login with social accounts</p>
                        <div className="line"></div>
                      </div>
